...
| cpu_pct | % | CPU usage (all threads combined, per-core: 800% = 8 cores) |
| rss_mb | MB | Resident Set Size |
| swap_mb | MB | Swap usage (from VmSwap in /proc/pid/status) |
...
| sim_speed | x | Simulation speed multiplier (1=normal, 2=2x, etc.) |

**Datarefs used:** `sim/operation/misc/frame_rate_period`,
`sim/time/gpu_time_per_frame_sec_approx`, `sim/flightmodel/position/*`,
`sim/time/total_running_time_sec`, `sim/time/paused`, `sim/time/sim_speed`.
