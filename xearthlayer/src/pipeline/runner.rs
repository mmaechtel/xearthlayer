...
use crate::pipeline::processor::{
    process_tile, process_tile_cancellable, process_tile_with_observer,
};
use crate::pipeline::{
...
///
/// This function:
/// 1. Checks coalescer for in-flight requests (coalescing logic)
/// 2. Submits work to control plane (job management)
    ...
    X: BlockingExecutor,
{
    ...
    // Note: Coalescing is handled separately from control plane.
    // For now, we submit directly to control plane. Request coalescing can be
    // layered on top by checking the coalescer before calling submit().
